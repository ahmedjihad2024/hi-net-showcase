...
String dataAmountFormat(DataUnit dataUnit, double dataRemainingMB) {
  return switch (dataUnit) {
    DataUnit.MB =>
      '${Translation.lefts_of_mb.trNamed({'lefts': (Decimal.parse(dataRemainingMB.toString())).toDouble().toStringAsFixed(1)})}',
    DataUnit.GB =>
      '${Translation.lefts_of_gb.trNamed({'lefts': (Decimal.parse(dataRemainingMB.toString()) / Decimal.parse('1024')).toDouble().toStringAsFixed(1)})}',
    DataUnit.TB =>
      '${Translation.lefts_of_tb.trNamed({'lefts': (Decimal.parse((Decimal.parse(dataRemainingMB.toString()) / Decimal.parse('1024')).toString()) / Decimal.parse('1024')).toDouble().toStringAsFixed(1)})}',
  };
}

double dataAmountFormatDouble(DataUnit dataUnit, double dataRemainingMB) {
  return switch (dataUnit) {
    DataUnit.MB =>
      (Decimal.parse(dataRemainingMB.toString())).toDouble(),
    DataUnit.GB =>
      (Decimal.parse(dataRemainingMB.toString()) / Decimal.parse('1024')).toDouble(),
    DataUnit.TB =>
      (Decimal.parse((Decimal.parse(dataRemainingMB.toString()) / Decimal.parse('1024')).toString()) / Decimal.parse('1024')).toDouble(),
  };
}
